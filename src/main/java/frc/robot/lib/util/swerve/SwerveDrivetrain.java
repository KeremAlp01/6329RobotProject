...
import static frc.robot.lib.util.swerve.SwerveRequest.SwerveControlRequestParameters;
    ...
    public void run() {
      ...
      while (m_running) {
        ...
        try {
          ...
          for (int i = 0; i < ModuleCount; ++i) {
            var pos = Modules[i].getPosition(false);
            if (!Double.isNaN(pos.distanceMeters) && !Double.isInfinite(pos.distanceMeters)) {
              m_modulePositions[i] = pos;
            } else {
              System.out.println("Invalid module position!! - Using cached pos!");
              m_modulePositions[i] = Modules[i].getCachedPosition();
            }
          }
          double yawDegrees =
          ...
          // DataLogManager.log(m_requestParameters.currentPose.toString());
